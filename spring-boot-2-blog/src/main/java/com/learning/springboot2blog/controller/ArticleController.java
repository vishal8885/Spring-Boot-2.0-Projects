package com.learning.springboot2blog.controller;

import com.learning.springboot2blog.exception.NotFoundException;
import com.learning.springboot2blog.model.Article;
import com.learning.springboot2blog.model.User;
import com.learning.springboot2blog.service.ArticleService;
import com.learning.springboot2blog.service.UserService;
...
public class ArticleController {
    ...
    public String savePost(
        ...
        if (article.getId() == null || article.getId().length() == 0) {
            ...
            Optional<Article> retrievedArticle = articleService.getById(article.getId());
            if(retrievedArticle.isPresent()) {
                article.setAuthor(retrievedArticle.get().getAuthor());
            }
        }
    ...
    @GetMapping("/rest")
    @ResponseBody
    public Page<Article> articlesRest(
            @RequestParam(required = false, value = "page") Integer page,
            @RequestParam(required = false, value = "size") Integer size
    ) {
    ...
    private void throwNotFoundException(@PathVariable String id) {
